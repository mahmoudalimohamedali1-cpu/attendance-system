...
let PermissionGuard = class PermissionGuard {
    ...
    async canActivate(context) {
        ...
        const hasPermission = await this.permissionsService.hasPermission(user.id, user.companyId, requiredPermission);
        if (!hasPermission) {
            throw new common_1.ForbiddenException(`ليس لديك صلاحية: ${requiredPermission}`);
        }
        return true;
    }
};
