...
export class ExcelService {
    ...
    async generatePayrollRunExcel(runId: string): Promise<Buffer> {
    ...
    async generateGosiReportExcel(month: number, year: number): Promise<Buffer> {
        // Fetch employees with their salaries and GOSI calculations
    ...
    /**
     * تقرير مقارنة الرواتب الشهرية
     */
    async generateSalaryComparisonExcel(month1: number, year1: number, month2: number, year2: number): Promise<Buffer> {
        ...
        const employeeMap = new Map<string, { name: string; code: string; net1: number; net2: number }>();
    ...
    /**
     * تقرير تكاليف الرواتب بالقسم
     */
    async generateDepartmentCostExcel(month: number, year: number): Promise<Buffer> {
        ...
        const deptMap = new Map<string, { name: string; count: number; base: number; ded: number; net: number }>();
     ...
     */
    async generateLoansReportExcel(): Promise<Buffer> {
