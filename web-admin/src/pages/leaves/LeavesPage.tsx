...
export const LeavesPage = () => {
  ...
  const createLeaveMutation = useMutation({
    mutationFn: (data: { type: string; startDate: string; endDate: string; reason: string }) =>
      api.post('/leaves', data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['leaves'] });
      queryClient.invalidateQueries({ queryKey: ['my-leave-balances'] });
      setOpenNewLeaveDialog(false);
      setNewLeaveForm({ type: 'ANNUAL', startDate: null, endDate: null, reason: '' });
  ...
  const handleSubmitNewLeave = () => {
    if (!newLeaveForm.startDate || !newLeaveForm.endDate || !newLeaveForm.reason) {
      setSnackbar({ open: true, message: 'يرجى تعبئة جميع الحقول المطلوبة', severity: 'error' });
      return;
    }
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={arSA}>
      <Box>
        ...
        <Dialog open={openNewLeaveDialog} onClose={() => setOpenNewLeaveDialog(false)} maxWidth="sm" fullWidth>
          <DialogTitle>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              <BeachAccess color="primary" />
              <Typography variant="h6">طلب إجازة جديدة</Typography>
            </Box>
          </DialogTitle>
          <DialogContent>
            <Box sx={{ pt: 2, display: 'flex', flexDirection: 'column', gap: 2.5 }}>
              ...
              <DatePicker
                ...
                onChange={(date) => setNewLeaveForm({ ...newLeaveForm, startDate: date })}
              ...
              <DatePicker
                ...
                onChange={(date) => setNewLeaveForm({ ...newLeaveForm, endDate: date })}
                minDate={newLeaveForm.startDate || undefined}
                slotProps={{ textField: { fullWidth: true } }}
              />
          ...
          <DialogActions sx={{ p: 2, gap: 1 }}>
            <Button onClick={() => setOpenNewLeaveDialog(false)} color="inherit">
              إلغاء
            </Button>
            <Button
              variant="contained"
              ...
              startIcon={createLeaveMutation.isPending ? <CircularProgress size={20} /> : <Add />}
            >
              إرسال الطلب
            </Button>
          </DialogActions>
