...
export class AiChatService {
    ...
    /**
     * 💬 إرسال رسالة للـ AI والحصول على رد
     */
    async chat(userId: string, message: string): Promise<{
        ...
        try {
            ...
            // 🧠 SMART ENHANCEMENT: Check FIRST before any other action detection
            // This has highest priority for system modification requests
            ...
            // 2. 🎯 تحليل الرسالة واكتشاف الأوامر
            const actionData = await this.detectAndExecuteAction(message, context);

            // ✅ إذا كان هناك بيانات من detectAndExecuteAction، ارجعها مباشرة
            ...
            // 🧠💡 GENIUS AI: Detect complex multi-module requests
            ...
            // ⚡ EXECUTIVE COMMANDS: Deploy, Backup, Monitor, Git, Logs
            ...
            if (this.agentTools.isOpenCreationRequest(message) &&
                ...
                // Use Genius Builder for complex requests, Full Auto for simple ones
            ...
            // 8. الحفاظ على آخر 20 رسالة فقط
            if (history.length > 20) {
                history.splice(0, history.length - 20);
            }
            this.conversationHistory.set(userId, history);

            // 9. استخراج اقتراحات للأسئلة التالية
    ...
    /**
     * 🔢 تحويل الأرقام النصية العربية إلى أرقام
     */
    private parseArabicNumber(text: string): number {
    ...
    /**
     * 🔍 استخراج اسم الموظف والقيمة من نص تعديل الراتب
     */
    private extractEmployeeAndValue(message: string, keyword: string): { employeeName: string; value: number } {
        // عدل راتب الموظف محمد سليم الي 10 الاف ريال
        // عدل راتب محمد سليم إلى 8000
        this.logger.log(`[EXTRACT] Original message: "${message}"`);

        // البحث عن الجزء بعد "راتب"
    ...
    /**
     * 🔍 استخراج اسم الموظف ونص من نص تعديل القسم
     */
    private extractEmployeeAndText(message: string, keyword: string): { employeeName: string; text: string } {
        const keywordIndex = message.indexOf(keyword);
        if (keywordIndex === -1) {
    ...
    /**
     * 🔍 استخراج اسم الموظف ورقم من نص إضافة إجازة
     */
    private extractEmployeeAndNumber(message: string): { employeeName: string; number: number } {
        // أضف إجازة لمحمد سليم 5 أيام
    ...
    /**
     * 🎯 اكتشاف وتنفيذ الأوامر - مع AI Agent Tools
     */
    private async detectAndExecuteAction(message: string, context: ChatContext): Promise<string> {
        try {
            this.logger.log(`[AI-CHAT] Detecting action for: "${message}"`);

            // للـ ADMIN و HR فقط - تحليل الأوامر
            if (context.userRole === 'ADMIN' || context.userRole === 'HR' || context.userRole === 'SUPER_ADMIN') {
                ...
                if ((message.includes('اعرض') || message.includes('قائمة') || message.includes('كل')) && message.includes('موظف')) {
                    const result = await this.agentTools.executeTool('list_employees',
                        { limit: 15 },
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                if ((message.includes('وافق') || message.includes('اقبل')) && message.includes('إجازة')) {
                    const name = this.extractNameFromMessage(message);
                    if (name) {
                        const result = await this.agentTools.executeTool('approve_leave',
                            { employeeName: name },
                            { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                        );
                        return result.message;
                ...
                if (message.includes('كم') && (message.includes('موظف') || message.includes('مهمة') || message.includes('إجازة'))) {
                    ...
                    const result = await this.agentTools.executeTool('query_count',
                        { entity },
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                if (message.includes('تقرير') && message.includes('حضور')) {
                    ...
                    const result = await this.agentTools.executeTool('attendance_report',
                        { period },
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                if (message.includes('متأخر') || message.includes('تأخر')) {
                    ...
                    const result = await this.agentTools.executeTool('late_employees',
                        { minLateCount: minLate },
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                }
                ...
                if ((message.includes('أعلى') || message.includes('اعلى') || message.includes('أقل')) && message.includes('راتب')) {
                    ...
                    const result = await this.agentTools.executeTool('top_salaries',
                        { count, order },
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                if ((message.includes('إحصائيات') || message.includes('احصائيات') || message.includes('ملخص')) && message.includes('إجاز')) {
                    const result = await this.agentTools.executeTool('leave_statistics',
                        {},
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                if ((message.includes('ملخص') || message.includes('إحصائيات')) && message.includes('حضور')) {
                    const result = await this.agentTools.executeTool('attendance_summary',
                        { period: 'today' },
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                // ==================== Phase 3: File Read Commands (SUPER_ADMIN) ====================
                if (context.userRole === 'SUPER_ADMIN') {
                    ...
                    if ((message.includes('اعرض') || message.includes('اقرأ')) && message.includes('ملف')) {
                        ...
                        if (fileMatch) {
                            const result = await this.agentTools.executeTool('read_file',
                    ...
                    if ((message.includes('الملفات') || message.includes('مجلد')) && (message.includes('في') || message.includes('modules'))) {
                        let dirPath = '';
                        if (message.includes('ai-chat')) dirPath = 'modules/ai-chat';
                        else if (message.includes('modules')) dirPath = 'modules';
                        else {
                            const dirMatch = message.match(/في\s+([^\s]+)/);
                            dirPath = dirMatch ? dirMatch[1] : '';
                        }
                        const result = await this.agentTools.executeTool('list_directory',
                            { dirPath },
                            { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                        );
                        return result.message;
                    ...
                    if (message.includes('ابحث') && message.includes('عن')) {
                        ...
                        if (searchMatch) {
                            const result = await this.agentTools.executeTool('search_code',
                                { query: searchMatch[1] },
                                { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                            );
                            return result.message;
                    ...
                    if ((message.includes('هيكل') || message.includes('functions')) && message.includes('ملف')) {
                        const fileMatch = message.match(/ملف\s+([^\s]+)/);
                        if (fileMatch) {
                            const result = await this.agentTools.executeTool('get_file_outline',
                                { filePath: fileMatch[1] },
                                { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                            );
                            return result.message;
                    ...
                    if (message.includes('أنشئ') && message.includes('ملف')) {
                        const fileMatch = message.match(/ملف\s+([^\s]+)/);
                        if (fileMatch) {
                            const result = await this.agentTools.executeTool('write_file',
                                { filePath: fileMatch[1], content: '// Auto-generated file\n' },
                                { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                            );
                            return result.message;
                        }
                    ...
                    if (message.includes('أنشئ') && message.includes('module')) {
                        ...
                        if (moduleMatch) {
                            const result = await this.agentTools.executeTool('create_module',
                    ...
                    if (message.includes('ريستارت') || message.includes('restart') || message.includes('أعد تشغيل')) {
                        const result = await this.agentTools.executeTool('restart_backend',
                            {},
                            { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                        );
                        return result.message;
                    ...
                    if (message.includes('حالة') && message.includes('git') || message.includes('git status')) {
                        const result = await this.agentTools.executeTool('git_status',
                            {},
                            { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                        );
                        return result.message;
                    ...
                    if (message.includes('معلومات') && message.includes('سيستم') || message.includes('system info')) {
                        const result = await this.agentTools.executeTool('system_info',
                            {},
                            { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                        );
                        return result.message;
                ...
                if (message.includes('توقع') && (message.includes('استقال') || message.includes('دوران'))) {
                    const result = await this.agentTools.executeTool('predict_turnover',
                        {},
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                if (message.includes('حلل') && message.includes('راتب') || message.includes('تحليل الرواتب')) {
                    const result = await this.agentTools.executeTool('salary_analysis',
                        {},
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                if (message.includes('شذوذ') || message.includes('غير طبيعي') || message.includes('anomaly')) {
                    const result = await this.agentTools.executeTool('anomaly_detection',
                        {},
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                if (message.includes('توزيع') && message.includes('مهام') || message.includes('workload')) {
                    const result = await this.agentTools.executeTool('workload_analysis',
                        {},
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                if (message.includes('أرسل') && message.includes('إشعار') && message.includes('لـ')) {
                    ...
                    if (match) {
                        const result = await this.agentTools.executeTool('send_notification',
                            { employeeName: match[1], message: msgMatch?.[1] || 'تنبيه' },
                            { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                        );
                        return result.message;
                    }
                ...
                if (message.includes('رسالة جماعية') || message.includes('broadcast')) {
                    ...
                    const result = await this.agentTools.executeTool('broadcast_message',
                ...
                if (message.includes('تذكير') || message.includes('reminder')) {
                    ...
                    const result = await this.agentTools.executeTool('create_reminder',
                        { title: titleMatch?.[1] || 'تذكير', date: new Date().toISOString().split('T')[0] },
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                }
                ...
                if (message.includes('تقرير ملخص') || message.includes('summary report')) {
                    const result = await this.agentTools.executeTool('send_summary_report',
                        { type: 'daily' },
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                // ==================== Phase 8: Integration Commands ====================

                // 📦 تصدير بيانات
                if (message.includes('صدر') && (message.includes('موظفين') || message.includes('حضور') || message.includes('إجازات'))) {
                    let dataType = 'employees';
                    if (message.includes('حضور')) dataType = 'attendance';
                    if (message.includes('إجازات')) dataType = 'leaves';
                    const result = await this.agentTools.executeTool('export_data',
                ...
                if (message.includes('إحصائيات') && (message.includes('قاعدة') || message.includes('بيانات'))) {
                    const result = await this.agentTools.executeTool('database_stats',
                        {},
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                if (message.includes('نسخ') && message.includes('احتياطي') || message.includes('backup')) {
                    const result = await this.agentTools.executeTool('backup_status',
                        {},
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                if (message.includes('صحة') && message.includes('api') || message.includes('health check')) {
                    const result = await this.agentTools.executeTool('api_health',
                        {},
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                if (message.includes('إجراءات سريعة') || message.includes('quick actions')) {
                    const result = await this.agentTools.executeTool('quick_actions',
                        {},
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                if (message.includes('اقتراحات') || message.includes('suggestions')) {
                    const result = await this.agentTools.executeTool('smart_suggestions',
                        {},
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                if (message.includes('مساعدة') || message.includes('help') || message.includes('الأوامر')) {
                    const result = await this.agentTools.executeTool('help_commands',
                        {},
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                // ✅ وافق على كل الإجازات
                if (message.includes('وافق') && message.includes('كل') && message.includes('إجاز')) {
                    const result = await this.agentTools.executeTool('batch_approve_leaves',
                        {},
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                if (message.includes('ملخص يومي') || message.includes('daily digest')) {
                    const result = await this.agentTools.executeTool('daily_digest',
                        {},
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                if (message.includes('تذكيرات تلقائية') || message.includes('auto reminder')) {
                    const result = await this.agentTools.executeTool('auto_reminder',
                        { type: 'tasks' },
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                if (message.includes('تقرير حضور') && message.includes('تفصيلي')) {
                    ...
                    const result = await this.agentTools.executeTool('attendance_detailed_report',
                ...
                if (message.includes('تفاصيل راتب') || message.includes('راتب') && message.includes('تفصيل')) {
                    ...
                    const result = await this.agentTools.executeTool('salary_breakdown',
                        { employeeName: nameMatch?.[1] || '' },
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                if (message.includes('تقرير قسم') || message.includes('قسم') && message.includes('تقرير')) {
                    ...
                    const result = await this.agentTools.executeTool('department_report',
                ...
                if (message.includes('مقارنة شهرية') || message.includes('قارن') && message.includes('شهر')) {
                    const result = await this.agentTools.executeTool('monthly_comparison',
                        {},
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                if (message.includes('مكافأة') || message.includes('bonus')) {
                    ...
                    if (nameMatch) {
                        const result = await this.agentTools.executeTool('add_bonus',
                            { employeeName: nameMatch[1], amount: parseInt(nameMatch[2]), reason: 'مكافأة' },
                            { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                        );
                        return result.message;
                    }
                ...
                if (message.includes('اخصم') || message.includes('خصم')) {
                    ...
                    if (nameMatch) {
                        const result = await this.agentTools.executeTool('add_deduction',
                            { employeeName: nameMatch[2], amount: parseInt(nameMatch[1]), reason: 'خصم' },
                            { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                        );
                        return result.message;
                    }
                ...
                if (message.includes('حالة الرواتب') || message.includes('payroll status')) {
                    const result = await this.agentTools.executeTool('payroll_status',
                        {},
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                if (message.includes('أوفرتايم') || message.includes('overtime')) {
                    ...
                    const result = await this.agentTools.executeTool('calculate_overtime',
                        { employeeName: nameMatch?.[1] || '' },
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                if (message.includes('أنشئ وردية') || message.includes('وردية جديدة')) {
                    ...
                    if (match) {
                        const result = await this.agentTools.executeTool('create_shift',
                            { name: match[1], startTime: match[2], endTime: match[3] },
                            { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                        );
                        return result.message;
                    }
                ...
                if (message.includes('اعرض الورديات') || message.includes('الورديات')) {
                    const result = await this.agentTools.executeTool('list_shifts',
                        {},
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
                ...
                if (message.includes('عيّن وردية') || message.includes('وردية') && message.includes('لـ')) {
                    ...
                    if (match) {
                        const result = await this.agentTools.executeTool('assign_shift',
                            { shiftName: match[1], employeeName: match[2] },
                            { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                        );
                        return result.message;
                    }
                ...
                if (message.includes('ورديات اليوم')) {
                    const result = await this.agentTools.executeTool('today_shifts',
                        {},
                        { companyId: context.companyId, userId: context.userId, userRole: context.userRole }
                    );
                    return result.message;
    ...
    /**
     * 📝 استخراج الاسم والقيمة من أمر التعديل
     */
    private parseUpdateCommand(message: string): { employeeName: string; value: string } {
        // عدل راتب الموظف محمد سليم الي 10 الاف ريال
        const separators = ['الي', 'إلى', 'الى', ' ل '];
        ...
        // استخراج الاسم - بين الكلمة المفتاحية والفاصل
    ...
    /**
     * 🆕 استخراج بيانات إضافة موظف
     */
    private parseAddEmployeeCommand(message: string): any {
        // أضف موظف جديد اسمه أحمد محمد في قسم IT براتب 5000
        const result: any = {};
    ...
    /**
     * 📝 استخراج بيانات إضافة مهمة
     */
    private parseTaskCommand(message: string): any {
        // أضف مهمة "إنهاء التقرير" للموظف أحمد
        const result: any = {};
    ...
    private async findEmployeeByName(name: string, companyId: string) {
        ...
        const scored = allEmployees.map(emp => {
            ...
            for (const part of nameParts) {
                if (emp.firstName?.toLowerCase().includes(part.toLowerCase())) {
                    score += 2; // تطابق في الاسم الأول
                }
                if (emp.lastName?.toLowerCase().includes(part.toLowerCase())) {
                    score += 2; // تطابق في الاسم الأخير
                }
            }

            // مكافأة للتطابق الكامل
        ...
        // لو الأعلى درجة فقط قليل، يعني مفيش تطابق كويس
     ...
     */
    private async getEmployeeFullData(name: string, companyId: string): Promise<string> {
...
━━━━━━━━━━━━━━━━━━━━
📋 المسمى: ${(emp as any).jobTitle || 'غير محدد'}
🏢 القسم: ${(emp as any).department || 'غير محدد'}
...
🎯 معرف الموظف: ${emp.id}
    ...
    /**
     * 🚪 تحليل معدل الدوران لموظف معين
     */
    private async analyzeEmployeeTurnover(name: string, companyId: string): Promise<string> {
        const employees = await this.findEmployeeByName(name, companyId);
        ...
        if (emp.hireDate) {
            ...
            if (monthsOfService < 6) {
                ...
                riskScore -= 10; // موظف قديم = أقل خطورة
            }
        }
        ...
        // 4. الإجازات (عدم أخذ إجازة = علامة على الإرهاق)
...
📌 يمكنك تعديل بيانات الموظف عبر الشات، مثال: "عدل راتب ${emp.firstName} إلى 8000"
     ...
     * ✏️ تعديل بيانات موظف
     */
    private async updateEmployeeField(
        name: string,
        field: string,
        value: string,
        companyId: string
...
✨ تم حفظ التغييرات في قاعدة البيانات.
     ...
     * 🏖️ إضافة أيام إجازة
     */
    private async addLeaveDays(name: string, days: number, companyId: string): Promise<string> {
        const employees = await this.findEmployeeByName(name, companyId);
        ...
        try {
            const currentDays = (emp as any).annualLeaveDays || 21;
...
✨ تم حفظ التغييرات.
     ...
     * 📊 جلب رصيد الإجازات
     */
    private async getLeaveBalance(userId: string): Promise<string> {
        ...
        if (!user) return 'لم يتم العثور على بيانات الإجازات';
...
- المتبقي: ${remaining} يوم
     ...
     * 📅 جلب حضور اليوم
     */
    private async getTodayAttendance(userId: string): Promise<string> {
        ...
        if (!attendance) return '📅 لم يتم تسجيل حضور اليوم بعد';

        const att = attendance as any; // Type assertion for flexibility
        return `
📅 سجل حضور اليوم:
...
- التأخير: ${att.lateMinutes || 0} دقيقة
     ...
     * 💰 جلب معلومات الراتب
     */
    private async getSalaryInfo(userId: string): Promise<string> {
        const user = await this.prisma.user.findUnique({
            where: { id: userId },
            select: {
                salary: true,
        ...
        if (!user) return 'لم يتم العثور على بيانات الراتب';
...
📌 ملاحظة: هذه الأرقام تقريبية. للتفاصيل الدقيقة راجع كشف الراتب.
     ...
     * 👥 جلب حالة الفريق
     */
    private async getTeamStatus(companyId: string): Promise<string> {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
...
- نسبة الحضور: ${attendanceRate}%
    ...
    /**
     * ⏰ جلب الموظفين المتأخرين
     */
    private async getLateEmployees(companyId: string): Promise<string> {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
...
🤖 ردك:`;
    ...
    /**
     * 💡 استخراج اقتراحات للأسئلة التالية
     */
    private extractSuggestions(context: ChatContext): string[] {
