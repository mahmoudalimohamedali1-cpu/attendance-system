...
// Exception Alert
const ExceptionAlert: React.FC<{ title: string; count: number; severity: 'error' | 'warning' | 'info' }> =
...
export const PayrollDashboardPage: React.FC = () => {
    ...
    const { data, isLoading, refetch } = useDashboard({ year, month });
                ...
                <Grid item xs={12} md={6}>
                    <Paper sx={{ p: 2 }}>
                        ...
                        <Box display="flex" flexWrap="wrap">
                            {health.attendance && <HealthBadge label="الحضور" status={health.attendance} />}
                            {health.leaves && <HealthBadge label="الإجازات" status={health.leaves} />}
                            {health.advances && <HealthBadge label="السلف" status={health.advances} />}
                            {health.policies && <HealthBadge label="السياسات" status={health.policies} />}
                            {health.gosiConfig && <HealthBadge label="GOSI" status={health.gosiConfig} />}
                            {health.payrollCalculated !== undefined && (
                                <HealthBadge label="محسوب" status={health.payrollCalculated} />
                            )}
                            {health.payrollLocked !== undefined && (
