...
export class AiPayrollService {
     ...
     */
    async detectSalaryAnomalies(companyId: string): Promise<SalaryAnomaly[]> {
        const anomalies: SalaryAnomaly[] = [];

        // جلب الموظفين مع بيانات الراتب
        ...
        for (const emp of employees) {
            ...
            const totalAdvances = advances.reduce((sum, a) => sum + Number((a as any).amount || 0), 0);
     ...
     * 💰 تقييم مخاطر السلفة
     */
    async assessAdvanceRisk(userId: string, requestedAmount: number): Promise<AdvanceRiskAssessment> {
        // جلب بيانات الموظف
    ...
    /**
     * 📊 تحسين الخصومات (نصائح AI)
     */
    async getDeductionOptimizations(userId: string): Promise<string> {
        try {
            const user = await this.prisma.user.findUnique({
                where: { id: userId },
                select: { salary: true },
...
قدم نصائح عملية بالعربية:`;
     ...
     */
    async getSalaryTrends(companyId: string): Promise<{
