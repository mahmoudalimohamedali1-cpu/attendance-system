 ...
 * Implements ideas #126-150: Multi-cultural & Language support
 * 
 * Features:
 * - Dialect support (Khaleeji, Masri, Shami)
 * - Cultural calendar (Ramadan, Eid, etc.)
...
@Injectable()
export class MultiCulturalService {
    private readonly logger = new Logger(MultiCulturalService.name);

    // Cultural events calendar
    private readonly culturalEvents: CulturalEvent[] = [
    ...
    // Expat services
    private readonly expatServices: ExpatService[] = [
        {
    ...
    // Dialect patterns
    private readonly dialectPatterns: Record<string, { name: string; examples: string[] }> = {
     ...
     * 📅 Get upcoming events
     */
    getUpcomingEvents(days: number = 30, country?: string): CulturalEvent[] {
        const now = new Date();
     ...
     * 🕌 Get Ramadan status
     */
    getRamadanStatus(): { isRamadan: boolean; daysUntil?: number; daysRemaining?: number; message: string } {
        const now = new Date();
     ...
     */
    detectDialect(text: string): { dialect: string; dialectAr: string; confidence: number } {
        let maxMatches = 0;
    ...
    /**
     * 🛂 Search expat services
     */
    searchExpatServices(query: string): ExpatService[] {
        const normalized = query.toLowerCase();
        return this.expatServices.filter(s =>
